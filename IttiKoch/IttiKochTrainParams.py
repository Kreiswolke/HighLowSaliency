...
class IttiKoch(object):
    def __init__(self, scale = 8, sigma = 1., windowsize = 8, orientations = [0., 45., 90., 135. ], ref_scale = 4, s_min_max= [2,5], deltas= [3,4], gabor_size = -1, extra_blur = False, norm_stepsize = 10):
        self.scale = scale
        self.sigma = sigma
    ...
    def orientation_features(self, _input):
        ...
        for angle in self.orientations:
            ...
            tmp1 = replication_padding(_input, axis=1, size=int(shape_0/2))
            tmp2 = replication_padding(tmp1, axis=2, size=int(shape_1/2))
    ...
    def get_intensity_conspicuity(self, net, ref_shape):
        normalized_net =  {k:self.map_normalization(v) for k,v in net.items()}
        net =  {k:tf.image.resize_images(v, ref_shape) for k,v in normalized_net.items()}
        return net

    def get_color_conspicuity(self, net, ref_shape ):
                
        normalized_net_RG =  {k:self.map_normalization(v) for k,v in net.items() if k.startswith('RG')}
        normalized_net_BY =  {k:self.map_normalization(v) for k,v in net.items() if k.startswith('BY')}
    ...
    def get_orientation_conspicuity(self, net, ref_shape):
        ...
        net = {}
        for angle in self.orientations:
            net[angle] = self.map_normalization(tf.add_n([v for k,v in normalized_net.items() if k.endswith(str(int(angle)))]))
    ...
    def build(self, _input):
        ...
        self.net = {'readout_features': {}, 'filters': {}, 'pre_features':{},'features':{}, 'conspicuity':{}}
        ...
        ####################################################################################################################
        # collect maps for readout network
        print(self.net['features']['intensity'])
        # Normalized intensity center surround features
        self.net['readout_features']['intensity'] = {k:repeat(self.map_normalization_tf(v), 2**(int(k.split('_')[2])-1)) for k,v in self.net['features']['intensity'].items()}
        
        # Normalized RG and BY center surround features
        self.net['readout_features']['color'] = {k:repeat(self.map_normalization_tf(v), 2**(int(k.split('_')[2])-1))  for k,v in self.net['features']['color'].items()}
        
        
        self.net['readout_features']['orientation'] = {k:repeat(self.map_normalization_tf(v), 2**(int(k.split('_')[2])-1)) for k,v in self.net['features']['orientation'].items()}
