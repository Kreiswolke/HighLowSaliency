...
def get_rgb2pca(image):
    ...
    X = []
    for row in V_pca:
        w1 = tf.cast(row[np.newaxis,np.newaxis,:], tf.float32)
        X.append(tf.reduce_sum(tf.mul(w1,tf.cast(image,tf.float32)),2))      
    return tf.pack(X,axis=2)


def get_rgb2pca_batch(image,bs):
    ''' Possibly rectifiy? '''
    V_pca = np.array([[+0.40052664, -0.7275945,  -0.55694224],
                      [-0.81574151, -0.00632331, -0.57838206],
                      [+0.41730589,  0.68597833, -0.59606168]])
    ...
    for row in V_pca:
        ...
        R = np.tile(row[np.newaxis,np.newaxis,:],(bs,1,1,1))
        w1 = tf.cast(R, tf.float32)
        M = tf.mul(w1,tf.cast(image,tf.float32))
        X.append(tf.reduce_sum(M,3))
    return tf.pack(X,axis=-1)


def get_rgb2pca_batch_v1(image,bs):
    ''' Possibly rectifiy? '''
    V_pca = np.array([[+0.40052664, -0.7275945,  -0.55694224],
                      [-0.81574151, -0.00632331, -0.57838206],
                      [+0.41730589,  0.68597833, -0.59606168]])
    ...
    for row in V_pca:
        ...
        w1 = tf.cast(R, tf.float32)
        try:
            M = tf.multiply(w1,tf.cast(image,tf.float32))
...
class MLC_tf(object):
    ...
    def __init__(self,  image_shape, color=True, sigmas=None, window_size=141, im_ds=1, downsample=1,
                 pooling_type='max'):
    ...
    def build_input_features_color(self, _input):
        ...
        net['input_input_size'] = _input
        if self.image_downsample == 2:
            print('MLC image downsampling by factor 2!')
            net['input'] = tf.nn.avg_pool(_input, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
        else: 
            print('MLC no image downsampling!')
            net['input'] = _input
        ...
        #net['PCA'] = get_rgb2pca(net['input']) #For 3dim input
        #net['PCA'] = get_rgb2pca_batch(net['input'], self.bs) #Self-implementation
        #Faster Version (hopefully)
        net['PCA'] = fast_PCA(net['input'], self.bs)
        ...
        for i in range(3):
            ...
            l_input =  net['PCA'][:,:,:,tf.constant(i)][:,:,:,tf.newaxis]
