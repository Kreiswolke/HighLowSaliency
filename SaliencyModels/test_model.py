...
def train(args):
    ...
    #save_dir = args.save_dir  + '/' +  args.flag + args.save_flag + cb_str + '/' 
    save_dir = args.save_dir  + '/' +  args.flag + args.save_flag + cb_str + '/test/fold_{}/'.format(args.fold)   
    ...
    #save_dir = args.save_dir  + '/' +  args.flag + args.save_flag + cb_str + '/test/fold_{}/'.format(args.fold)   
    save_str = save_dir + '/' + args.flag + '_batch_{}{}_fold_{}.p'.format(args.bs, cb_str, args.fold)
    ...
    with g.as_default():
        model = Model(args,height,width, training=False)
        with tf.Session() as test_sess:   
            ...
            res_dict = {k: {'gamma': None, 'ckpt_dir': args.ckpt_dir, 'im_ds': args.im_ds,'ds': args.ds, 'sigmas':args.sigmas, 'inds_dir': args.inds_dir, 'cb_dir': args.cb_dir, 'steerdict':args.steer_dict, 'flag':args.flag,'lr': None, 'inds':valid_inds,'sal_map': None, 'im':None, 'xinds':None,'yinds':None, 'binds': None, 'epoch_loss':None, 'run_loss': []} for k in range(args.epochs)}
            ...
            for k in range(5):
                ...
                for i,j in enumerate(batch_ids):
                    ...
                    loss = test_sess.run(model.dg_mean, init_feed_dict)
